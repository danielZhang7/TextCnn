...
def train():
    ...
    tensorboard_dir = './tensorboard/' + project_name
    ...
    require_improvement = 3000  # break training if not having improvement over 1000 iter
    flag = False
        ...
        for x_batch, y_batch in batch_train:
            ...
            if global_step % config.print_per_batch == 0:
                ...
                if (val_accuracy > best_val_accuracy) or (
                        val_accuracy == best_val_accuracy and train_accuracy > best_train_accuracy) or (
                        val_accuracy == best_val_accuracy and train_accuracy == best_train_accuracy and train_loss < best_train_lost) or (
                        val_accuracy == best_val_accuracy and train_accuracy == best_train_accuracy and best_train_lost == train_loss and val_loss < best_val_lost):

                    saver.save(session, save_path)
                    best_val_accuracy = val_accuracy
                    best_train_accuracy = train_accuracy
                    best_val_lost = val_loss
                    best_train_lost = train_loss
...
if __name__ == '__main__':
    print('Configuring CNN model for ' + project_name + '...')
    config = TextConfig(project_name)

    # train word2vec
    if not os.path.exists(config.vector_word_filename):
        filenames = [config.train_filename, config.test_filename, config.val_filename]
        train_word2vec(config.vector_word_filename, filenames)
